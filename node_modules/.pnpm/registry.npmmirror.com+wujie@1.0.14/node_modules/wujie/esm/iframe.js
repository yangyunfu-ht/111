...
export function insertScriptToIframe(scriptResult, iframeWindow, rawElement) {
  ...
  var code = jsLoader(content, src, getCurUrl(proxyLocation)); // 添加属性
  ...
  container.appendChild(scriptElement); // 调用回调

  callback === null || callback === void 0 ? void 0 : callback(iframeWindow); // 执行 hooks

  execHooks(plugins, "appendOrInsertElementHook", scriptElement, iframeWindow, rawElement); // 内联脚本执行后的处理
...
export function iframeGenerator(sandbox, attrs, mainHostPath, appHostPath, appRoutePath) {
  ...
  var iframeWindow = iframe.contentWindow; // 变量需要提前注入，在入口函数通过变量防止死循环
